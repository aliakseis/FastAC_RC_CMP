...
#include <stdlib.h>
#include <windows.h> //this is for timestamp
...
int main(int numb_arg, char * arg[])
{
  ...
  if ((numb_arg != 4) || (arg[1][0] != '-') || 
     ((arg[1][1] != 'c') && (arg[1][1] != 'd'))) {
    puts("\n\t Compression parameters:   fastac -c data_file compressed_file");
    puts("\n\t Decompression parameters: fastac -d compressed_file new_file\n");
    exit(0);
  }
