...
//                                                                           -
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
// The functions write_to_output_buffer and read_from_inpit_buffer 
// were added by Andrew Polar http://www.ezcodesample.com November 2006
// The purpose is to write and read data from/to buffer rather than from/to 
// file and provide all operations through the memory.
...
unsigned Arithmetic_Codec::read_from_input_buffer(unsigned char* buffer, int offset) 
{
  unsigned shift = 0, code_bytes = 0;
  int file_byte;
                      
  do {
    file_byte = buffer[offset];
    code_bytes |= unsigned(file_byte & 0x7F) << shift;
    shift += 7;
	offset++;
  } while (file_byte & 0x80);
      
  memcpy(code_buffer, buffer + offset, code_bytes); 
  offset += code_bytes;	
...
unsigned Arithmetic_Codec::write_to_output_buffer(unsigned char* buffer, int offset) {
  ...
  do {
	...
	buffer[offset + header_bytes] = file_byte; 	
    header_bytes++;
  } while (nb);

  memcpy(buffer + offset + header_bytes, code_buffer, code_bytes);
  return offset + code_bytes + header_bytes;  // bytes used
}
